...
io.sockets.on('connection', function(socket){
	...
	socket.on('frame_to_server', function(data){
		...
		var name = 'public/imagesForGallery/'+fullDate+'.webp';
...
function base64_decode(file_name, image){
	...
	console.log("Hi");
	fs.writeFile(image, file_name, function(err){
		if(err){
			console.log(err);
		} else{
			console.log("Success");
...
app.get('/gallery', function(req, res, err){
	...
	fs.readdir('public/imagesForGallery', (err, files) => {
		if (err) {
			throw err;
		}
		image = files;
	...
	for(var i = image.length-1; i >=0; i--){
		res.write('<img width = "315" height="315" style="border-color: #111111; border-style: solid; display: inline-block;float: left;margin: 5px 5px;"src="imagesForGallery/'+image[i]+'"/>');
	}
	res.end();
	console.log(files);
	});
